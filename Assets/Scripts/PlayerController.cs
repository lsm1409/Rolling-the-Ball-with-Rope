...
{
    public float JumpPower = 1.5f; // 공 점프시 가하는 힘
    public float MovePower = 20.0f; // 공 이동시 가하는 힘
    public float MaxVelocity = 18.0f; // 공의 최고속도 (공이 이 속도 이하일 때만 힘을 가한다.)
    public float MaxAngularVelocity = 30.0f; // 공의 최고 각속도 (공이 공 중심 기준으로 회전하는 최고 속도)
    ...
    private bool isConnected = false;   // 공이 로프와 연결되어있는지 여부
    private HingeJoint rope;   // 로프의 joint 정보 (로프가 걸리는 위치, 로프의 길이, 진자 운동 방향, 진자 운동 각도 등)
    private RaycastHit ropeHit;    // 로프가 닿은 오브젝트의 정보
    ...
    private void Update()
    {
        isJumpPressed = ropeJoystick._isJumped; // 로프 조이스틱에서 점프 키를 눌렸는지 여부를 가져온다.
        isRopeAimed = ropeJoystick._isAimed; // 로프 조이스틱이 조준 상태인지 여부를 가져온다.
        isRopeShot = ropeJoystick._isShot;    // 로프 조이스틱에서 로프 발사가 입력되었는지 여부를 가져온다.
